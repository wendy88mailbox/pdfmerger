...
def main():
    ...
    try:
        ...
        # 分頁：上傳 vs 合併
        tab1, tab2 = st.tabs(["📤 上傳檔案", "📑 合併 PDF"])
        
        # ========== Tab 1: 上傳檔案 ==========
        with tab1:
            st.subheader("📤 上傳檔案到 Google Drive")
            st.markdown(f"上傳照片或 PDF 到「**{folder['name']}**」資料夾")
            ...
            if uploaded_files:
                ...
                # 顯示檔案列表
                with st.expander("查看檔案清單", expanded=True):
                ...
                if st.button("🚀 開始上傳", type="primary", use_container_width=True):
                    ...
                    for idx, file in enumerate(uploaded_files):
                        status_text.text(f"⬆️ 上傳中: {file.name}...")
                    ...
                    if fail_count > 0:
                        st.warning(f"⚠️ **{fail_count}** 個檔案上傳失敗")
                        with st.expander("查看失敗檔案"):
