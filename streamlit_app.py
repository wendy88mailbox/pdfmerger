        ...
        if merge_button:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if merge_pdfs(pdf_files, output_path):
                    # 上傳到服務帳號的 Google Drive
                    status_text.text("⬆️ 上傳到 Google Drive...")
                    progress_bar.progress(0.9)
                    
                    uploaded_file = upload_file(service, output_path)
                    ...
                    # 顯示下載連結
                    st.info(f"📎 **檔案已上傳到 Google Drive**")
                    st.markdown(f"### [🔗 點此開啟檔案]({uploaded_file['webViewLink']})")
                    st.caption("檔案儲存在服務帳號的 Google Drive 中，任何人都可以檢視")
