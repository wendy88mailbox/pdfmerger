...
def main():
    ...
    with st.sidebar:
        ...
        # 檔名設定
        st.subheader("📝 輸出檔名")
        use_date = st.checkbox("加上日期", value=True)
        custom_name = st.text_input("自訂前綴", value="合併檔案")
        
        st.markdown("---")
        st.info("💡 **提示**\n\n選擇要合併的檔案後，點擊「開始合併」按鈕。")
    ...
    try:
        ...
        st.subheader(f"📁 檔案清單 ({len(supported_files)} 個檔案)")
        ...
        for idx, file in enumerate(supported_files):
            ...
            with col1:
                if select_all:
                    selected = st.checkbox(f"選擇 {idx+1}", value=True, key=f"select_{idx}", label_visibility="collapsed")
                else:
                    selected = st.checkbox(f"選擇 {idx+1}", value=False, key=f"select_{idx}", label_visibility="collapsed")
